...
def display_temperatureC(img, val_k, loc, color):
    ...
    #val = ktof(val_k)
    val = ktoc(val_k)
    cv2.putText(img,"{0:.1f} oC".format(val), loc, cv2.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)
    x, y = loc
    cv2.line(img, (x - 2, y), (x + 2, y), color, 1)
    cv2.line(img, (x, y - 2), (x, y + 2), color, 1)
...
def getFrame():
    ...
    display_temperatureC(img, maxVal, maxLoc, (0, 0, 255))
    display_temperatureC(img, minVal, minLoc, (255, 0, 0))
    #cv2.imwrite('/home/uno/flask-opencv-streaming/t.jpg',img)
    return img
